...
long int
schedule_packet_arrival_event(Simulation_Run_Ptr simulation_run, Server_Ptr link,
			      double event_time)
{
  Event event;
  ...
  // add the event in the schedule depending on the time
...
void
packet_arrival_event(Simulation_Run_Ptr simulation_run, Server_Ptr link)
{
  Simulation_Run_Data_Ptr data;
  Packet_Ptr new_packet;
  double ARRIVAL_RATE_LIST[] = {PACKET_ARRIVAL_RATE, 0 };

  // returns a list of data to a pointer to a struct
  ...
  new_packet->destination_id = uniform_generator();
  ...
  schedule_packet_arrival_event(simulation_run, data->link,
			simulation_run_get_time(simulation_run) +
			exponential_generator((double) 1/ ARRIVAL_RATE_LIST[i]));
}
