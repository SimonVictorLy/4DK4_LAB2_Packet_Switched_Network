...
end_packet_transmission_event(Simulation_Run_Ptr simulation_run, void * link)
{
  ...
  // If the third server is complete, send packet to the other queues
  if (i==2) {
		
	  if (this_packet->destination_id <= (double) PROBABILITY) {
		  (data - 1)->arrival_count++;
		  if (server_state((data - 1)->link) == BUSY) fifoqueue_put((data - 1)->buffer, (void*)this_packet);
		  else start_transmission_on_link(simulation_run, (void*)this_packet, (data - 1)->link);
	  }
	  else {
		  (data - 2)->arrival_count++;
		  if (server_state((data - 2)->link) == BUSY) fifoqueue_put((data - 2)->buffer, (void*)this_packet);
		  else start_transmission_on_link(simulation_run, (void*)this_packet, (data - 2)->link);
	  }
  }
  else xfree((void *)this_packet); /* This packet is done ... give the memory back. */
 ...
 // else if (i = 2) packet_arrival_event(simulation_run, (data-1)->link);
}
...
// call end packet transmission event 
void
			   ...
			   Server_Ptr link)
{
  ...
  schedule_end_packet_transmission_event(simulation_run,
	 ...
	 (void *) link);
  // go back to execute event after end transmission event (main loop)
}
